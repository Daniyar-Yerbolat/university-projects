...
void* input(buffer_t * in){
	...
	while(x < N_DATA){
		...
		fprintf(fp, "input buffer : data %d is %d\n", x+1, data);
		clock_gettime( CLOCK_REALTIME, &start[x]);
		x++;
		}
...
while(x < N_DATA){
	...
	fprintf(fp, "out buffer : data %d is %d\n", x+1, data);
	clock_gettime( CLOCK_REALTIME, &stop[x]);
	x++;
	}
}
}
...
while(x <N_DATA){
    lat = ( stop[x].tv_sec - start[x].tv_sec )
          + ( stop[x].tv_nsec - start[x].tv_nsec )
            / BILLION;
if(x == 0){
min_lat = lat;
max_lat = lat;
