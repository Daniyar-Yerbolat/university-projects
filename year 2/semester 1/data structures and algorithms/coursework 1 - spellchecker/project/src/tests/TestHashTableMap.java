...
public class TestHashTableMap {
	...
	/**
	 * Runs the hash table at different load factors and print out the average
	 * probe numbers versus the running time. The average probe number should go
	 * up as the max load factor goes up.
	 */
	private static void runDifferentLoadFactors() {
		...
		while (maxLF < 0.99) {
			...
			try {
				...
				System.out.println(
						String.format("For load factor %9f, average num. of  probes is %9f time in milseconds is %9f",
								maxLF, h.averNumProbes(), time));
